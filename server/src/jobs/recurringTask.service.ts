...
import { createNotifications, NotificationPayload } from '../shared/services/notification.service';
...
const SUPPORTED_PATTERNS = ['daily', 'weekly', 'monthly', 'yearly'];
...
// Add months, keeping the original day-of-month but clamped to the target month's length.
// Jan 31 +1 → Feb 28 → Mar 31 (re-anchored by the caller); Feb 29 +12 → Feb 28; Dec → Jan rollover.
...
const getDayOfMonth = (template: any): number => {
    const dom = Number(template.recurringDayOfMonth);
    if (Number.isInteger(dom) && dom >= 1 && dom <= 31) return dom;
    return 1;
};

// First spawn slot for monthly: recurringDayOfMonth in the start month if not already passed, else next month.
const firstSelectedDayOfMonthOnOrAfter = (date: Date, dayOfMonth: number): Date => {
    const year = date.getFullYear();
    const month = date.getMonth();
    const daysInMonth = new Date(year, month + 1, 0).getDate();
    const day = Math.min(dayOfMonth, daysInMonth);
    if (day >= date.getDate()) return new Date(year, month, day);
...
// Monthly slot STRICTLY after the anchor: next month re-anchored to recurringDayOfMonth,
// so a clamped end-of-month anchor (Feb 28) still yields Mar 31, never Mar 28.
const nextMonthlySlot = (after: Date, dayOfMonth: number): Date => {
...
export const nextSpawnDate = (template: any, lastSpawnedAt: Date | null): Date | null => {
    ...
    if (!lastSpawnedAt) {
        ...
        if (template.recurringPattern === 'monthly') {
            const first = firstSelectedDayOfMonthOnOrAfter(start, getDayOfMonth(template));
            return applySpawnTime(first, hours, minutes);
        }
        if (template.recurringPattern === 'yearly') {
            return applySpawnTime(start, hours, minutes);
        }
        return null;
    }
    ...
    switch (template.recurringPattern) {
        ...
        case 'monthly': {
            const next = nextMonthlySlot(last, getDayOfMonth(template));
            return applySpawnTime(next, hours, minutes);
        }
        case 'yearly': {
            const next = addMonthsClamped(last, 12);
            return applySpawnTime(next, hours, minutes);
        }
        default:
            return null;
